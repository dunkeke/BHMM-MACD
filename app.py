...
class HMM_MACD_Strategy:
    """
    [新增] HMM + 4H MACD 贝叶斯共振策略
    逻辑：
    1. 计算 4小时 MACD 作为“技术面似然函数” (Likelihood)
    2. 计算 日线 HMM 作为“结构面先验概率” (Prior)
    3. 贝叶斯更新 -> 后验概率 -> 信号
    """
    def __init__(self, n_components=3, iter_num=1000, window_size=21):
        self.n_components = n_components
    ...
    def get_4h_macd_signal(self, ticker):
        """获取4小时级别数据并计算MACD"""
        try:
            # yfinance 限制：1h数据只能取730天。我们取最近60天的高频数据来模拟近期信号
            # 对于更久远的历史回测，这里简化使用日线MACD模拟，或需付费数据源
            # 这里为了演示"当前状态"，我们取最近60天1h数据并重采样
            ...
            # 重采样为 4小时 K线
            df_4h = df_1h['Close'].resample('4h').ohlc() # 这里的4h是固定时间窗口
            df_4h.dropna(inplace=True)
            close_4h = df_4h['close'] # 注意是小写close

            # 计算 MACD
            macd, signal, hist = self.calculate_macd(close_4h)
            
            # 对齐到日线 (取当天的最后一个4h信号作为日线信号的修正)
            # 这是一个降采样过程
    ...
    def generate_signals(self, df, ticker_symbol=None):
        ...
        df['Prob_S0_Prior'] = df.get('Prob_S0', 0.33) # 假设 S0 是看涨状态
        df['Prob_S2_Prior'] = df.get('Prob_S2', 0.33) # 假设 S2 是看跌状态
        
        # 2. 获取 4H MACD 证据 (Evidence)
        if ticker_symbol:
        ...
        # 似然函数 P(MACD | State): 
        # - 如果 MACD 金叉 (Hist > 0)，更支持 State 0 (牛)
        # - 如果 MACD 死叉 (Hist < 0)，更支持 State 2 (熊)
        
        # 似然强度因子 (MACD Influence)
        # 归一化 MACD 直方图，使其在 -1 到 1 之间 (粗略估计)
        macd_norm = np.clip(df['MACD_Hist_4H'] / (df['Close'] * 0.01 + 1e-6), -1, 1) * 2.0 # 放大系数
        
        # 计算后验概率
        # P(S0|MACD) ∝ P(S0) * exp(macd_norm)  (MACD越正，S0概率越大)
        # P(S2|MACD) ∝ P(S2) * exp(-macd_norm) (MACD越负，S2概率越大)
        # P(S1|MACD) ∝ P(S1) * 1             (MACD对震荡状态无特别偏好)
        ...
        # 4. 更新信号
        # 如果后验 S0 概率 > 50% 且 MACD > 0 -> Strong Buy
        # 如果后验 S2 概率 > 50% 且 MACD < 0 -> Strong Sell
        # 否则 -> Wait
...
def display_signal_panel(df, strategy_type):
    ...
    with col_reason:
        ...
        if "MACD" in strategy_type:
            ...
            msg = f"""
            - **HMM 宏观概率 (后验)**: 牛(S0): **{prob_0_post:.1f}%** | 熊(S2): **{prob_2_post:.1f}%**
            - **4H 技术面共振**: MACD Histogram = {macd_val:.3f} ({macd_status})
            """
            if sig == 1: msg += "\n\n💡 **结论**: 宏观低波动 + 4H级别MACD金叉共振，**强烈看多**。"
            elif sig == -1: msg += "\n\n💡 **结论**: 宏观高波动 + 4H级别MACD死叉共振，**强烈看空**。"
            else: msg += "\n\n💡 **结论**: HMM 与 MACD 信号背离或动能不足，建议**观望**。"
            st.info(msg)
